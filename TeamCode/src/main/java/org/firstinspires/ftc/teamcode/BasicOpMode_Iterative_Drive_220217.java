...
@TeleOp(name="Mechanum Drive New Robot 02/17", group="Iterative Opmode")
//@Disabled
public class BasicOpMode_Iterative_Drive_220217 extends OpMode
{
    ...
    public void loop() {
        ...
        if (gamepad2.left_trigger > trigger) {
                ...
                // gamepad2 y
        }else if (gamepad2.right_trigger > trigger) {
            armObj.setPower(0);
            armObj.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            armObj.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            armObj.setTargetPosition(7171);
            armObj.setPower(1);
            setWristPosition(0.3);
            if (LinearArmObj.getCurrentPosition() >= 1565) {
                LinearArmObj.setPower(0);
            } else {
                LinearArmObj.setPower(0.7);
            }
